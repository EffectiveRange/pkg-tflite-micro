...
void FftCompute(struct FftState *state, const int16_t *input,
  ...
  kiss_fft_fixed16::kiss_fftr(
      reinterpret_cast<kiss_fft_fixed16::kiss_fftr_cfg>(state->scratch),
      state->input,
      reinterpret_cast<kiss_fft_fixed16::kiss_fft_cpx *>(state->output));
}
/**/
  kiss_fft_fixed8::kiss_fftr(
      reinterpret_cast<kiss_fft_fixed8::kiss_fftr_cfg>(state->scratch),
      state->input,
      reinterpret_cast<kiss_fft_fixed8::kiss_fft_cpx *>(state->output));
}
