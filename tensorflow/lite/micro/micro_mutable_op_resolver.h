...
public:
  ...
  GetOpDataParser(BuiltinOperator op) const override {
  ...
  TfLiteStatus AddCustom(const char *name,
    ...
    if (registrations_len_ >= tOpCount) {
      MicroPrintf("Couldn't register custom op '%s', resolver size is too"
                  "small (%d)",
                  name, tOpCount);
      return kTfLiteError;
    }
  ...
  TfLiteStatus
  AddConv2D(const TFLMRegistration &registration = Register_CONV_2D()) {
  ...
  TfLiteStatus
  AddMaxPool2D(const TFLMRegistration &registration = Register_MAX_POOL_2D()) {
  ...
  TfLiteStatus
  AddSoftmax(const TFLMRegistration &registration = Register_SOFTMAX()) {
  ...
  TfLiteStatus
  AddUnidirectionalSequenceLSTM(const TFLMRegistration &registration =
